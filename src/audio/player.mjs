...
export class Player {
  ...
  async init() {
    ...
    this.bus = buildMasterBus(this.ctx, { seed: this.noiseSeed });
  }
  ...
  async load(piece) {
    ...
    this.stop();
    // 种子变了就重建总线，让混响 IR 也跟着换（否则换种子后混响不变）
    if (this.ctx && this.noiseSeed !== piece.seed) {
      this.noiseSeed = piece.seed;
      try { this.bus.master.disconnect(); } catch {}
      this.bus = buildMasterBus(this.ctx, { seed: this.noiseSeed });
    }
  }
  ...
  _tick() {
    ...
    if (this.onProgress) this.onProgress(this.position, this.piece.totalSeconds);
  }
}
