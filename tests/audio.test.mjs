...
import {
  playNoteOn, buildMasterBus, prepareEvents, createReverbIR, midiToHz,
...
function makeParam(name, log) {
  ...
  const p = {
    ...
    set value(v) {
      if (typeof v !== 'number' || Number.isNaN(v)) throw new Error(`${name} 直接赋值不是数字: ${v}`);
      _v = v;
      log.push(['setValue', name, v]);
    },
    setValueAtTime(v, t) { check(name, v, t); log.push(['setValueAtTime', name, v, t]); return p; },
    linearRampToValueAtTime(v, t) { check(name, v, t); log.push(['linearRamp', name, v, t]); return p; },
    exponentialRampToValueAtTime(v, t) {
      if (v === 0) throw new Error(`exponentialRampToValueAtTime 不能到 0（${name}）`);
      check(name, v, t); log.push(['expRamp', name, v, t]); return p;
    },
    setTargetAtTime(v, t) { check(name, v, t); return p; },
  };
  function check(n, v, t) {
    if (typeof v !== 'number' || Number.isNaN(v)) throw new Error(`${n} 的值不是数字: ${v}`);
    if (typeof t !== 'number' || Number.isNaN(t)) throw new Error(`${n} 的时间不是数字: ${t}`);
...
function makeNode(kind, log) {
  const node = {
    _kind: kind,
    connect(dest) { log.push(['connect', kind, dest?._kind ?? 'dest']); return dest; },
    disconnect() {},
  };
  return node;
}
...
function makeCtx({ sampleRate = 44100 } = {}) {
  ...
  const ctx = {
    ...
    createOscillator() {
      ...
      n.start = (t) => { if (t < 0) throw new Error('osc.start 时间为负'); log.push(['osc.start', t]); };
      n.stop = (t) => { if (t < 0) throw new Error('osc.stop 时间为负'); log.push(['osc.stop', t]); };
      return n;
    },
    createBiquadFilter() {
    ...
    createDynamicsCompressor() {
      ...
      for (const k of ['threshold', 'knee', 'ratio', 'attack', 'release']) n[k] = makeParam(k, log);
      return n;
    },
...
test('总线构建成功且接线合理', () => {
  ...
  assert.ok(bus.input && bus.master && bus.comp && bus.limiter && bus.convolver);
  assert.ok(bus.convolver.buffer, '卷积器必须已载入 IR');
  const kinds = ctx._log.filter((l) => l[0] === 'connect').map((l) => `${l[1]}→${l[2]}`);
    ...
    for (const timbre of ['kick', 'snare', 'hat', 'clap']) {
      ...
      assert.doesNotThrow(() => {
        playNoteOn(ctx, bus.input, {
          voice, timbre, midi: 60, velocity: 0.8, duration: 0.5, startTime: 0.25,
        });
      }, `${voice}/${timbre} 构造失败`);
    }
  }
...
test('振荡器频率与音高一致（不会把 midi 当 Hz）', () => {
  const ctx = makeCtx();
  const bus = buildMasterBus(ctx);
  playNoteOn(ctx, bus.input, { voice: 'melody', midi: 69, velocity: 1, duration: 0.4, startTime: 0 });
  const anyFreq = ctx._log.filter(
    (l) => (l[0] === 'setValueAtTime' || l[0] === 'setValue') && l[1] === 'frequency',
  );
...
test('包络不使用非法的指数斜坡到 0', () => {
  ...
  assert.doesNotThrow(() => {
    playNoteOn(ctx, bus.input, { voice: 'pad', midi: 60, velocity: 0, duration: 3, startTime: 0 });
  });
});
...
test('零力度、零时长等边界输入不产生 NaN', () => {
  ...
  for (const ev of [
    ...
    { voice: 'perc', timbre: 'hat', midi: 42, velocity: 0, duration: 0, startTime: 5 },
  ]) {
    assert.doesNotThrow(() => playNoteOn(ctx, bus.input, ev), JSON.stringify(ev));
  }
});
...
test('prepareEvents：区间内事件被平移到 0 起点', () => {
  ...
  assert.deepEqual(out.map((e) => e.startTime), [0]);
...
test('prepareEvents：跨过起点的长音被保留并从 0 开始', () => {
  ...
  assert.ok(Math.abs(out[0].duration - 7) < 1e-9, `剩余时长应为 7，实际 ${out[0].duration}`);
});

test('prepareEvents：不重复计入（回归：曾把跨起点音符算两次）', () => {
  const events = [{ time: 0, duration: 6, voice: 'bass', midi: 40, velocity: 1 }];
...
test('任意区间切分都不越界、不产生负时间', () => {
  ...
  for (const start of [0, 1.3, 5, piece.totalSeconds * 0.5, piece.totalSeconds - 0.5]) {
