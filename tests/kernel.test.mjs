...
} from "../src/core/ambient.mjs";
import { registry, ambientPack } from "../src/content/builtin.mjs";
import { ContentRegistry } from "../src/content/registry.mjs";
import {
  beat,
  beatToSeconds,
  edoPitch,
...
test("声部音区严格隔离（无重叠）", () => {
  ...
  assert.ok(
    byVoice.bass.length > 0 && byVoice.melody.length > 0,
    "应同时有低音与旋律",
  );
  assert.ok(
    Math.max(...byVoice.bass) < Math.min(...byVoice.harmony),
    "低音必须低于和声",
  );
  assert.ok(
    Math.max(...byVoice.harmony) < Math.min(...byVoice.melody),
    "和声必须低于旋律",
  );
});
...
test("密度不失控", () => {
  ...
  assert.ok(
    piece.stats.densityPeaks < piece.events.length * 0.05,
    "密度尖峰过多",
  );
});
...
test("能量参数真实影响事件数量", () => {
  ...
  assert.ok(
    loud.events.length > quiet.events.length * 1.3,
...
test("情绪参数影响音高分布（走向更高）", () => {
  ...
  assert.ok(
    avg(bright) > avg(dark),
    `明亮应更高：dark=${avg(dark).toFixed(1)}, bright=${avg(bright).toFixed(1)}`,
  );
});
...
test("段落密度包络：break 段比 main 段更疏", () => {
  ...
  const barsOf = (name) =>
    piece.config.sections.find((s) => s.name === name).bars;
...
test("19-EDO 的频率与音分偏差非零（验证非 12-TET 可用）", () => {
  ...
  assert.ok(
    Math.abs(t.centsFrom12Tet(70)) > 1,
    "19-EDO 的音级应对 12-TET 有偏差",
  );
});
...
test("全部内置氛围场景都有稳定 id，且可生成可听片段", () => {
  assert.ok(AMBIENT_SCENES.length >= 8, "试点应覆盖至少八个场景");
  assert.equal(
    new Set(AMBIENT_SCENES.map((scene) => scene.id)).size,
    AMBIENT_SCENES.length,
    "场景 id 必须唯一",
  );
  for (const scene of AMBIENT_SCENES) {
    const piece = generateAmbientSegment({
      seed: "ambient",
      sceneId: scene.id,
    });
    assert.equal(piece.ambient.sceneId, scene.id);
    assert.ok(piece.events.length > 30, `${scene.name} 没有生成足够事件`);
    assert.notEqual(
      piece.stats.verdict,
      "poor",
      `${scene.name} 生成了 poor 结果`,
    );
  }
});

test("氛围场景配置返回副本，调用方修改不会污染内容注册表", () => {
  const a = ambientConfig("reading");
  const b = ambientConfig("reading");
  a.sections[0].name = "被修改";
  a.arrangement.melody.startSteps[0] = 99;
  assert.notEqual(b.sections[0].name, "被修改");
  assert.notEqual(
    b.arrangement.melody.startSteps[0],
    99,
    "嵌套编配数据也必须隔离",
  );
});

test("阅读与休息从第 0 拍开始持续铺底，且不再包含呼吸段", () => {
  for (const sceneId of ["reading", "resting"]) {
    const piece = generateAmbientSegment({ seed: "continuous-bed", sceneId });
    assert.equal(piece.events[0].time, 0, `${sceneId} 的第一拍不应静音`);
    assert.equal(
      piece.config.sections.some((section) => section.name === "呼吸"),
      false,
      `${sceneId} 不应包含可感知的呼吸段`,
    );
    for (const section of piece.config.sections) {
      const continuityBed = piece.events.find(
        (event) =>
          event.section === section.name && event.timbre === "continuityPad",
      );
      assert.ok(continuityBed, `${sceneId}/${section.name} 缺少连续底床`);
      assert.ok(
        continuityBed.duration > piece.barSeconds * section.bars,
        `${sceneId}/${section.name} 的底床应跨过段落边界`,
      );
    }
  }
});

test("场景编配真正改变音色、节奏和空间，而非只换 BPM", () => {
  const pieces = Object.fromEntries(
    AMBIENT_SCENES.map((scene) => [
      scene.id,
      generateAmbientSegment({ seed: "scene-profile", sceneId: scene.id }),
    ]),
  );
  const arrangementIds = new Set(
    Object.values(pieces).map((piece) => piece.arrangementId),
  );
  const melodyTimbres = new Set(
    Object.values(pieces).flatMap((piece) =>
      piece.events
        .filter((event) => event.voice === "melody")
  ...
  assert.ok(melodyTimbres.size >= 7, "场景应使用可感知差异的旋律音色");
  assert.equal(
    pieces.reading.events.some((event) => event.voice === "perc"),
    false,
    "阅读默认不使用显性打击",
  );
  assert.equal(
    pieces.resting.events.some((event) => event.voice === "perc"),
    false,
    "休息默认不使用显性打击",
  );
  assert.ok(
    pieces.party.events.filter((event) => event.voice === "perc").length >
      pieces.dining.events.filter((event) => event.voice === "perc").length * 4,
    "派对应比吃饭拥有更明确的节拍层",
  );
  assert.ok(
    pieces.resting.mix.reverbSeconds > pieces.workout.mix.reverbSeconds,
    "休息的空间尾音应比运动更长",
  );
});

test("无尽片段种子与片段内容严格可复现", () => {
  assert.equal(ambientSegmentSeed("seed", "reading", 0), "seed");
  assert.equal(
    ambientSegmentSeed("seed", "reading", 2),
    ambientSegmentSeed("seed", "reading", 2),
  );
  assert.notEqual(
    ambientSegmentSeed("seed", "reading", 1),
    ambientSegmentSeed("seed", "reading", 2),
  );
  const a = generateAmbientSegment({
    seed: "seed",
    sceneId: "driving",
    segmentIndex: 3,
  });
  const b = generateAmbientSegment({
    seed: "seed",
    sceneId: "driving",
    segmentIndex: 3,
  });
...
test("无尽时间线只向后拼接，事件时间有序且不重叠", () => {
  const first = generateAmbientSegment({
    seed: "flow",
    sceneId: "reading",
    segmentIndex: 0,
  });
  const second = generateAmbientSegment({
    seed: "flow",
    sceneId: "reading",
    segmentIndex: 1,
  });
  const timeline = appendAmbientSegment(first, second);
  assert.equal(
    timeline.events.length,
    first.events.length + second.events.length,
  );
  assert.equal(timeline.totalSeconds, first.totalSeconds + second.totalSeconds);
  assert.ok(
    timeline.events.every(
      (event, i, events) => i === 0 || event.time >= events[i - 1].time,
    ),
  );
  const appended = timeline.events.slice(first.events.length);
  assert.ok(appended.every((event) => event.time >= first.totalSeconds));
  assert.equal(timeline.ambient.segmentCount, 2);
});

test("内置氛围包以精确 id@version 注册，并可从 Snapshot 解析", () => {
  const snapshot = makePieceSnapshot({
    packId: "@tunehub/ambient",
    packVersion: "1.0.0",
    sceneId: "reading",
  ...
  const resolved = registry.resolveSnapshot(snapshot);
  assert.equal(resolved.pack.manifest.id, "@tunehub/ambient");
  assert.equal(resolved.scene.id, "reading");
  assert.throws(
    () => registry.resolve("@tunehub/ambient", "9.9.9"),
    /缺少内容包/,
  );
});

test("内容 registry 拒绝不完整包与重复精确版本", () => {
  const local = new ContentRegistry();
  assert.throws(() => local.register({ manifest: {} }), /缺少/);
  local.register(ambientPack);
  assert.throws(() => local.register(ambientPack), /已注册/);
});

test("规范 Score 保留分数 Beat、TempoMap 与 Pitch 投影", () => {
  assert.equal(beatToSeconds(beat(4), [{ at: beat(0), bpm: 120 }]), 2);
  assert.equal(pitchToMidi(edoPitch(69)), 69);
  const score = validateScore({
    ...
    events: [{ type: "marker", at: beat(0), kind: "section", label: "intro" }],
  });
  assert.equal(score.events[0].type, "marker");
});
...
test("声部覆盖：只改变目标声部，其余声部逐事件不变", () => {
  ...
  const rolled = generate("locktest", { energy: 0.7 }, undefined, {
    melody: "newmel",
  });
...
test("声部覆盖不改变曲式结构", () => {
  ...
  const rolled = generate("structtest", {}, undefined, {
    melody: "zzz",
    perc: "yyy",
  });
...
test("覆盖后的作品仍然好听且不越界", () => {
  for (let i = 0; i < 25; i++) {
    const piece = generate(`ov${i}`, { energy: 0.9 }, undefined, {
      melody: `m${i}`,
      perc: `p${i}`,
    });
    assert.ok(
      piece.stats.verdict !== "poor",
      `覆盖后出现 poor: ${JSON.stringify(piece.stats)}`,
    );
    for (const e of piece.events) {
      if (e.voice === "perc") continue;
      const v = VOICES[e.voice];
