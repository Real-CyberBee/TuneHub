...
| M3 第一个游戏 | ⬜ 未开始 |
| M4 内容生态 | 🟡 schema + 校验器已就绪（`examples/`），未接入运行时 |
...
## M4 · 内容生态与贡献飞轮 — 🟡 格式与校验器已就绪

| 任务 | 验收 |
|---|---|
| 内容 Schema（7 种 kind）+ `provenance` 必填校验 | 无出处内容进不来 |
| **浏览器内预设/风格编辑器** | 不离开浏览器即可创建并试听 |
| 一键"提交到内容库"（自动生成 PR） | 带 schema 校验 |
| 内容包加载（`?pack=`） | 无需重新构建 |
| 可视化贡献模板 + 沙箱 + 性能预算 | 第三方可视化不威胁主页面 |
| 多语言内容名 + 策展列表 | 默认只加载精选内容 |

**验收**：社区能**只写 JSON** 贡献一个风格，且立即在网站上听到。
...
| `Framework` 数据模型 + 少量样例 | 作为**能力演示**，非产品主张 |
